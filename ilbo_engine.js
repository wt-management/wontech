...
(function(root){
'use strict';
var DEPT_MAP={'A0402040':'국내영업','A0402020':'국내영업','A0402030':'국내영업','A0402010':'국내영업','A0402050':'국내영업','A0401020':'국내영업','A0401040':'국내영업','A0401070':'국내영업','B04020020':'국내영업','B04020030':'국내영업','A0401030':'해외영업','A04010040':'해외영업','A0401050':'Surgical','B0901030':'Surgical','B04030020':'Surgical','A0501020':'고객만족','A0501030':'고객만족','A0501010':'고객만족','A05010040':'고객만족','A05010020':'고객만족','A0401060':'B2C','A04000020':'B2C','A0701020':'기타'};
...
// 소계행만 걸러낸다. 예전엔 '적요가 [ 로 시작하면 소계'로 뭉뚱그렸는데,
// 그 바람에 [결산] 반품충당부채 6건(-10.8억)까지 통째로 빠져 일보와 어긋났다.
var SUB=/^\s*\[\s*(전\s*기\s*이\s*월|월\s*계|누\s*계|합\s*계|당\s*기\s*누\s*계)\s*\]/;
...
function parseAccounting(wb,fname){
  ...
  wb.SheetNames.forEach(function(sn){
    ...
    for(var i=1;i<rows.length;i++){
      ...
      if(typeof memo==='string'&&SUB.test(memo)) continue;
      if(!String(cname||'').trim()) continue;   // 거래처 없는 행(반복 헤더·구분선)은 매출이 아니다
  ...
  var _dm=lines.map(function(l){return l.month;}).filter(Boolean); var _maxMo=_dm.length?Math.max.apply(null,_dm):curMo; var _curYr=rd?parseInt(rd.slice(0,4),10):new Date().getFullYear(); var _now=new Date(); if(curMo>_maxMo && !(_curYr===_now.getFullYear()&&curMo===_now.getMonth()+1)) curMo=_maxMo; // 당월(달력 현재 연·월)은 데이터가 없어도 캡하지 않고 즉시 롤오버; 과거 월만 데이터 최신월로 캡 — wt-consumables/ilbo.html의 parseAccounting과 동기화(2026-09 드리프트 발견)
